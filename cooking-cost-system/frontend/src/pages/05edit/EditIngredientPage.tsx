...
const EditIngredientPage: React.FC = () => {
    ...
    const validate = () => {
        ...
        if (!formData.name.trim()) { newErrors.name = '入力してください'; isValid = false; }
        if (!formData.priceAfter.trim()) {
            newErrors.priceAfter = '入力してください'; isValid = false;
        } else if (!isHalfWidthNumber(formData.priceAfter)) {
            newErrors.priceAfter = '半角数字で入力してください'; isValid = false;
        }
        if (!formData.quantity.trim()) {
            newErrors.quantity = '入力してください'; isValid = false;
        } else if (!isHalfWidthNumber(formData.quantity)) {
            newErrors.quantity = '半角数字で入力してください'; isValid = false;
        }
        if (!formData.supplier_id) { newErrors.supplier_id = '選択してください'; isValid = false; }
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (name === 'priceAfter' || name === 'quantity') {
    ...
    const handleFinalSubmit = async () => {
        ...
        try {
            ...
            if (response.success) {
                ...
                setFormData({ id: null, name: '', priceBefore: '0', priceAfter: '', quantity: '', unit: 'g', supplier_id: '', genre_id: '' });
                setSearchQuery('');
                setErrors({});
    ...
    const parseVoiceData = useCallback((text: string) => {
        ...
        words.forEach((word) => {
            ...
            if (priceMatch) { data.priceAfter = priceMatch[1]; return; }
    ...
    const startListening = () => {
        ...
        if (!SpeechRecognition) { toast.error('お使いのブラウザは音声認識に対応していません。'); return; }
    ...
    const IngredientEditIcon = () => (
        <svg viewBox="0 0 100 100" className="w-9 h-9 text-white" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M30 20 H70 C72.2 20 74 21.8 74 24 V76 C74 78.2 72.2 80 70 80 H30 C27.8 80 26 78.2 26 76 V24 C26 21.8 27.8 20 30 20Z" stroke="currentColor" strokeWidth="3.5" strokeLinejoin="round" />
            ...
            <path d="M55 58 C55 52 62 48 68 48 C74 48 81 52 81 58 C81 68 74 74 68 74 C62 74 55 68 55 58Z" fill="currentColor" stroke="#10b981" strokeWidth="1.5" />
            <path d="M68 48 Q70 42 74 40" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" />
            <path d="M78 25 L88 35 L80 43 L70 33 L78 25Z" fill="currentColor" stroke="#10b981" strokeWidth="1" />
            ...
            <div className="max-w-6xl mx-auto">
                <div className="flex flex-col md:flex-row gap-12 items-start">
                    ...
                    <div className="w-full md:w-2/3 space-y-8">
                        ...
                        <AnimatePresence>
                            {lastTranscript && (
                                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="bg-purple-50 border border-purple-100 rounded-2xl p-4 flex items-center gap-3">
