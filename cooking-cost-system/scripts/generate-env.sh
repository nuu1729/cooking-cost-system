...
# ⚠️  環境変数名（キー）を追加・変更する際は backend/.env.example も合わせて更新すること。
#     APP_ENV の有効値（development/test/staging/production）を変更する場合は
#     backend/app.py の _VALID_APP_ENVS も更新すること。
readonly -a ALLOWED_ENV_KEYS=(APP_ENV PORT DATABASE_URL_PRODUCTION JWT_SECRET SECRET_KEY CORS_ORIGIN)
...
validate_env_file() {
    ...
    # このスクリプトは .env.production 専用のため APP_ENV の値によらず常に CORS を検証する
    # （APP_ENV が誤設定されていても検証をスキップしないようにする）
...
rotate_secrets() {
    ...
    case "${current_app_env}" in
        development|test|staging|production)
            ...
            ;;
        *)
            log_warn "APP_ENV の値が想定外です: '${current_app_env}'。production を使用します。"
            current_app_env=""
            ;;
    esac
